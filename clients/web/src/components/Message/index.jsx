...
const Message = ({ messageId, sender, content, timestamp, isReceived, location, isOnline, currentUsername, likesCount: initialLikesCount = 0, likedByCurrentUser, image }) => {
  ...
  const handleLikeClick = async () => {
    ...
    try {
      ...
      if (response.success) {
        console.log(`Message ${messageId} like toggled. New state: ${response.liked ? 'liked' : 'unliked'}`);
        
        // Update local state
        setLiked(response.liked);
        setLikesCount(response.likesCount);
        
        // Force refresh of profile data in case it's a toggle from liked to unliked,
        // which would affect liked messages in profile
        if (window.refreshProfileData && typeof window.refreshProfileData === 'function') {
          window.refreshProfileData();
        }
      } else {
        console.error('Failed to toggle message like:', response.message);
      }
    } catch (error) {
